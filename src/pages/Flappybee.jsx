 ...
 *
 * 🐝 FLAPPY TWOBEE (/flappybee) — variante "gioco" della coppia di landing A/B.
 * Al posto del calcolatore ROI (che faceva scappare chi non conosce i propri numeri)
 * c'è un mini-gioco: un'apina gialla che schiva i pain point dell'imprenditore medio.
...
const VARIANT = 'flappy'

/* ------------------------------------------------------------------ */
/* COPY it/en — SOLO i pezzi specifici del gioco (hero, pains, lose,    */
/* gate, faq). Il copy condiviso vive in ./landingShared.jsx.           */
/* ------------------------------------------------------------------ */
const COPY = {
  it: {
    hero: {
...
/* PAGE                                                                 */
/* ================================================================== */
export default function FlappybeePage() {
...
function Experience() {
  ...
  const handleStart = () => track('game_start', { page: 'flappybee' })
  ...
  const goToCalendar = () => {
    ...
    track('landing_book_cta', { variant: VARIANT, source: 'game' })
    setOver(false)
    setStage('lose')
    setGameKey((k) => k + 1)
    setTimeout(scrollToBooking, 90)
  }
  const handleRestart = () => {
    setOver(false)
    setStage('lose')
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-brand-dark/80 p-6 shadow-2xl sm:p-8">
      ...
      <div className="mt-7 rounded-2xl bg-brand-yellow p-5 text-center text-brand-black sm:p-7">
        <h4 className="font-display text-xl font-extrabold leading-tight sm:text-2xl">{call.heading}</h4>
...
/* ------------------------------------------------------------------ */
/* COSA RICEVI + CHI SIAMO — page-local, copy a tema-gioco.             */
/* ------------------------------------------------------------------ */
function WhatYouGet() {
  const lang = useLang()
...
/* ------------------------------------------------------------------ */
/* CTA FINALE — page-local, torna al gioco in cima.                    */
/* ------------------------------------------------------------------ */
function FinalCta() {
  const lang = useLang()

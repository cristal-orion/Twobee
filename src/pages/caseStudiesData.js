...
export const CASES = [
  ...
  },
  {
    ...
    // Prodotto nostro, non cliente: sul sito si sceglie settore e scenario e la
    // chiamata arriva davvero.
    urlLabel: { it: 'Prova SofIA al telefono', en: 'Try SofIA on the phone' },
    year: '2026',
    ...
    about: {
      it: 'SofIA è un prodotto nostro: un centralino con voce artificiale per le attività che vivono di telefonate — studi dentistici, centri estetici, palestre, ristoranti, concessionarie.',
      en: 'SofIA is a product of ours: an AI-voice switchboard for the businesses that live on the phone — dental practices, beauty salons, gyms, restaurants, car dealers.',
    },
    diagnosis: {
      it: 'In quelle attività il telefono suona mentre sei con un cliente. Nessuno risponde, e chi chiamava passa al nome successivo dell’elenco: una chiamata persa non lascia traccia da nessuna parte, non finisce in un report, è solo un incasso che non è arrivato — e non sai nemmeno di averlo perso. Chi riesce a rispondere scrive l’appuntamento su un’agenda di carta, e a fine giornata nessuno sa quante chiamate siano arrivate né cosa chiedessero. Una persona in segreteria copre gli orari d’ufficio: non le sere e i fine settimana, che sono le ore in cui la gente chiama.',
      en: 'In those businesses the phone rings while you are with a customer. Nobody picks up, and the caller moves on to the next name on the list: a missed call leaves no trace anywhere, it never reaches a report, it is simply money that did not arrive — and you don’t even know you lost it. Whoever does answer writes the appointment in a paper diary, and by the end of the day nobody knows how many calls came in or what they were about. A receptionist covers office hours: not the evenings and weekends, which are the hours when people actually call.',
    },
    meta: [
      {
        label: { it: 'Settore', en: 'Industry' },
